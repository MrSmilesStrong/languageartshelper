...
class Game {
    ...
    updateContinueButton() {
        ...
        if (night6Completed === 'true') {
            ...
            this.customNightBtn.classList.remove('hidden'); // é€šå…³Night 6åŽè§£é”Custom Night
        } else {
    ...
    // Continueæ¸¸æˆï¼ˆä»Žä¿å­˜çš„å…³å¡å¼€å§‹ï¼‰
    async continueGame() {
        if (this.loadProgress()) {
            this.mainMenu.classList.add('hidden');
            ...
            // ç›´æŽ¥å¼€å§‹æ¸¸æˆï¼Œä¸æ’­æ”¾è¿‡åœºåŠ¨ç”»
            await this.initGame();
        }
    }
    ...
    async startGame() {
        ...
        const endCutscene = () => {
            ...
            // ç­‰å¾…æ·¡å‡ºå®ŒæˆåŽéšè—å¹¶å¼€å§‹æ¸¸æˆ
            setTimeout(() => {
                cutscene.classList.add('hidden');
                cutscene.classList.remove('fade-out');
                // ä¸åœ¨è¿™é‡Œæ˜¾ç¤ºæ¸¸æˆç”»é¢ï¼Œè®©initGameå¤„ç†
                this.initGame();
            }, 3000);
        ...
        // 3ç§’åŽè‡ªåŠ¨å¼€å§‹æ·¡å‡ºï¼ˆæ€»å…±6ç§’ï¼š3ç§’æ·¡å…¥ + 3ç§’æ·¡å‡ºï¼‰
    ...
    async initGame() {
        // console.log('ðŸŽ® initGame called, currentNight:', this.state.currentNight);
        ...
        // console.log('ðŸŽ® After state.reset(), currentNight:', this.state.currentNight);
        
        // é‡ç½®æ‘„åƒå¤´ç³»ç»Ÿçš„soundæŒ‰é’®è®¡æ•°
        this.camera.resetSoundButtonCount();
        
        // æ¢å¤æ‘„åƒå¤´é¢æ¿çš„displayï¼ˆä¹‹å‰å¯èƒ½è¢«å¼ºåˆ¶éšè—ï¼‰
        const cameraPanel = document.getElementById('camera-panel');
        if (cameraPanel) {
            cameraPanel.style.display = ''; // æ¢å¤é»˜è®¤
            // console.log('ðŸŽ® Camera panel display restored');
        }
        
        // æ˜¾ç¤ºæ¯æ™šå¼€å§‹åœºæ™¯ï¼ˆåœ¨æ˜¾ç¤ºæ¸¸æˆç”»é¢ä¹‹å‰ï¼‰
        await this.showNightIntro();
        
        // console.log('ðŸŽ® After showNightIntro(), currentNight:', this.state.currentNight);
        
        // è¿›åœºåŠ¨ç”»ç»“æŸåŽæ‰æ˜¾ç¤ºæ¸¸æˆç”»é¢
        ...
        // åˆå§‹åŒ–é£Žæ‰‡çŠ¶æ€ï¼ˆé€šé£Žå£é»˜è®¤æ‰“å¼€ï¼Œé£Žæ‰‡å¿«é€Ÿæ—‹è½¬ï¼‰
        ...
        // console.log('ðŸŽ® Before Golden check, currentNight:', this.state.currentNight);
        
        // Night 5: å¿…å®šè§¦å‘ Golden éœé‡‘å½©è›‹ï¼ˆæ”¾åœ¨æœ€åŽï¼Œç¡®ä¿æ¸¸æˆå·²å®Œå…¨åˆå§‹åŒ–ï¼‰
        if (this.state.currentNight === 5) {
            // console.log('ðŸŒŸ Night 5 detected, triggering Golden Stephen...');
            setTimeout(() => {
                this.showGoldenStephen();
            }, 1000); // è¿›å…¥æ¸¸æˆ1ç§’åŽè§¦å‘
        } // else {
            // console.log('âŒ Not Night 5, currentNight is:', this.state.currentNight);
        // }
    }
    
    // åˆå§‹åŒ–é£Žæ‰‡åŠ¨ç”»çŠ¶æ€
    initVentFanAnimation() {
        const ventIcon = document.querySelector('.vent-icon');
        if (ventIcon) {
            if (this.state.ventsClosed) {
                // é€šé£Žå£å…³é—­ï¼Œé£Žæ‰‡åœæ­¢
                ventIcon.classList.add('stopped');
                ventIcon.style.animation = 'none';
            } else {
                // é€šé£Žå£æ‰“å¼€ï¼Œé£Žæ‰‡å¿«é€Ÿæ—‹è½¬
    ...
    // Golden éœé‡‘å½©è›‹æ•ˆæžœ
    showGoldenStephen() {
        console.log('ðŸŒŸ Golden Stephen Hawking appears!');
        
        // åˆ›å»ºå…¨å±é‡‘è‰²éœé‡‘å›¾å±‚
    ...
    toggleVents() {
        ...
        if (this.state.ventsClosed) {
            // å½“å‰å…³é—­ï¼Œè¦æ‰“å¼€ -> é£Žæ‰‡ä»Žåœæ­¢åŠ é€Ÿåˆ°å¿«é€Ÿ
            ...
            // å½“å‰æ‰“å¼€ï¼Œè¦å…³é—­ -> é£Žæ‰‡ä»Žå¿«é€Ÿå‡é€Ÿåˆ°åœæ­¢
        ...
        setTimeout(() => {
            ...
            // é€šçŸ¥ EnemyAI é€šé£Žå£çŠ¶æ€å˜åŒ–
    ...
    toggleCamera() {
        // console.log('ðŸŽ® Game.toggleCamera() called');
        // console.log('ðŸŽ® Current state - cameraOpen:', this.state.cameraOpen, 'tutorialActive:', this.state.tutorialActive);
        this.camera.toggle();
    }
    ...
    winNight() {
        ...
        // å…³é—­æ‘„åƒå¤´ï¼ˆå¦‚æžœæ‰“å¼€ï¼‰
        if (this.state.cameraOpen) {
            this.camera.close();
        }
        
        // å¼ºåˆ¶éšè—æ‘„åƒå¤´é¢æ¿ï¼Œé˜²æ­¢é—ªçŽ°
        const cameraPanel = document.getElementById('camera-panel');
        if (cameraPanel) {
            cameraPanel.classList.add('hidden');
            cameraPanel.classList.remove('show', 'closing');
            cameraPanel.style.display = 'none'; // å¼ºåˆ¶éšè—
        }
        
        // ç«‹å³éšè—æ¸¸æˆç”»é¢ï¼Œé˜²æ­¢é—ªçƒ
        this.gameScreen.classList.remove('active');
        ...
        if (this.state.customNight && this.state.currentNight === 7) {
            ...
            if (levels.epstein === 20 && levels.trump === 20 && levels.hawking === 20) {
                console.log('ðŸŒŸ 20/20/20 Custom Night completed!');
                localStorage.setItem('customNight202020', 'true');
            }
        }
        
        // å¦‚æžœæ˜¯ Night 6ï¼Œæ’­æ”¾ç‰¹æ®Šçš„èƒœåˆ©åŠ¨ç”»å¹¶æ ‡è®°å®Œæˆ
        if (this.state.currentNight === 6) {
            ...
            // å…¶ä»–å…³å¡æ’­æ”¾æ™®é€šçš„å¤œæ™šç»“æŸåŠ¨ç”»
    ...
    playNight5VictoryAnimation() {
        ...
        // 1ç§’åŽå˜ä¸º 6:00 AM å¹¶æ’­æ”¾é’Ÿå£°
        setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                // 2ç§’åŽæ·¡å‡º "RESCUE ARRIVE"ï¼Œæ˜¾ç¤ºèƒœåˆ©ç”»é¢
                setTimeout(() => {
                    rescueText.style.opacity = '0';
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                // Night 5 é€šå…³åŽï¼Œè§£é” Night 6ï¼ˆSpecial Nightï¼‰
    ...
    playNight6VictoryAnimation() {
        ...
        // 3ç§’åŽæ·¡å‡ºæ—¶é—´ï¼Œæ˜¾ç¤ºnight6.pngå›¾ç‰‡
        setTimeout(() => {
            timeDisplay.style.transition = 'opacity 0.5s';
            timeDisplay.style.opacity = '0';
    ...
    playNightEndAnimation() {
        ...
        setTimeout(() => {
            // æ·¡å‡ºæ—¶é—´ï¼ˆä¸æ”¹å˜å®¹å™¨é€æ˜Žåº¦ï¼Œä¿æŒé»‘è‰²èƒŒæ™¯ï¼‰
            timeDisplay.style.transition = 'opacity 0.5s';
            timeDisplay.style.opacity = '0';
            ...
            setTimeout(() => {
                ...
                // å¦‚æžœè¿˜æœ‰ä¸‹ä¸€å…³ï¼Œæ˜¾ç¤ºå‰©ä½™å¤©æ•°ï¼ˆæ•…äº‹è®¾å®šæ˜¯5æ™šï¼Œæ‰€ä»¥æ€»æ˜¯æ˜¾ç¤º5-å½“å‰å…³å¡ï¼‰
                    ...
                    // æ‰€æœ‰å…³å¡å®Œæˆï¼Œæ˜¾ç¤ºTO BE CONTINUED
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    // Custom Night é€šå…³åŽè¿”å›žä¸»èœå•
                    if (this.state.customNight && this.state.currentNight === 7) {
                        this.showMainMenu();
                    }
                    // å¦‚æžœè¿˜æœ‰ä¸‹ä¸€å…³ï¼Œç›´æŽ¥è¿›å…¥ä¸‹ä¸€å…³
                    else if (this.state.currentNight < this.state.maxNights) {
                        this.state.currentNight++;
                        this.continueToNextNight();
                    } else {
                        // æ‰€æœ‰å…³å¡å®Œæˆï¼Œæ¸…é™¤è¿›åº¦å¹¶è¿”å›žä¸»èœå•
                        this.clearProgress();
                        this.showMainMenu();
                    }
                }, 500);
            }, 3000); // æ”¹ä¸º3ç§’ï¼Œè®©çŽ©å®¶æœ‰æ—¶é—´çœ‹æ¶ˆæ¯
        }, 3000);
    }
    ...
    gameOverScreen(message, win = false) {
        ...
        if (win) {
            ...
            // ä¿å­˜è¿›åº¦ï¼ˆå¦‚æžœåœ¨Night 2æˆ–æ›´é«˜å…³å¡æ­»äº¡ï¼‰
            this.saveProgress();
    ...
    async continueToNextNight() {
        ...
        // æ¢å¤æ‘„åƒå¤´é¢æ¿çš„displayï¼ˆä¹‹å‰è¢«å¼ºåˆ¶éšè—ï¼‰
        const cameraPanel = document.getElementById('camera-panel');
        if (cameraPanel) {
            cameraPanel.style.display = ''; // æ¢å¤é»˜è®¤
        }
        ...
        // Night 5: å¿…å®šè§¦å‘ Golden éœé‡‘å½©è›‹
        if (this.state.currentNight === 5) {
            console.log('ðŸŒŸ Night 5 detected (continueToNextNight), triggering Golden Stephen...');
            setTimeout(() => {
                this.showGoldenStephen();
            }, 1000);
        }
    ...
    restartGame() {
        ...
        // å¦‚æžœæ˜¯Custom Nightï¼Œé‡æ–°å¼€å§‹Custom Night
        if (this.state.customNight && this.state.currentNight === 7) {
